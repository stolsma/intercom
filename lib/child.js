...
Child.prototype.start = function(restart) {
	...
	if (!child) {
		this.child = null;
		process.nextTick(function() {
			EventEmitter2.prototype.emit.call(self, 'error', new Error('Target script does not exist: ' + self.script));
		});
		return this;
	}
	...
	child.on('exit', function(code) {
		...
		function restartChild() {
			...
			process.nextTick(function() {
				EventEmitter2.prototype.emit.call(this, 'warn', 'Forever restarting script for ' + self.times + ' time');
				self.start(true);
			});
		}
...
Child.prototype.emit = function emitHandler(event) {
	...
	// eventlist! Array.prototype.slice.call(arguments)
