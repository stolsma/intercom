...
EventCom.prototype.startRpc = function(messenger) {
  ...
  // if the weak module is avaiable use it to minimize the chance of memory leaks
  var weakOptions;
  if (weak) {
    weakOptions = {
      wrap: function(cb, id) {
  ...
  var session = self.rpcSession = dnodeProtocol({
    emit: function(event) {
  ...
  messenger.on('disconnect', function() {
    self.stopRpc();
    self.localEmit('disconnected', 'internal');
  });


  self.on('disconnect', function() {
    if (self.started) {
      // the Child_process module can already have closed the IPC but the 'disconnect' message is still in the queue
      // so first check if the IPC is already disconnected, and if so just wait for the 'disconnect' message
