...
EventChild.prototype.tryFork = function() {
	...
	// create a copy of the options array and add the script to call in front
	var options = this.options.slice(0);
	options.unshift(this.script);

	try {
		return fork(path.join(__dirname, 'intercom.js'), options, this.spawnWith);
	} catch (err) {
		return false;
	}
};
