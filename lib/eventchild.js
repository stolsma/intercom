...
var EventChild = module.exports = function(script, options) {
  ...
  this.execArgv  = options.execArgv || null;
...
EventChild.prototype.start = function(restart) {
  ...
  process.nextTick(function() {
    self.localEmit(restart ? 'restart' : 'start', self.data);
  });
  ...
  // listen to child error event end emit on self
  child.on('error', function() {
    self.localEmit.apply(self, arguments);
  });

  // listen to child exit event end emit on self
  child.on('exit', function(code, signal) {
    ...
    function letChildDie() {
      ...
      self.localEmit('close', spinning, code, signal);
    }
    ...
    self.localEmit('exit', code, signal);
...
EventChild.prototype.tryFork = function() {
  ...
  this.spawnWith.execArgv = this.execArgv || this.spawnWith.execArgv;
  this.spawnWith.silent   = typeof this.silent !== 'undefined' ? this.silent : this.spawnWith.silent;
